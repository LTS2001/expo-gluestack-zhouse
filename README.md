...
>
> [🔔 语雀版README](https://www.yuque.com/u39373291/pspxx6/fhbhuddae4lpaoff)
>
> -------------------------------------------------------
>
...
配置好上面的文件之后运行`npm run android:dev`/`npm run android:prod`命令，会根据运行的命令来读取`env.dev`/`env.prod`自动生成`configs/env.ts`文件
...
```json
{
  "scripts": {
    ...
    // 运行命令android:dev之前自动执行的命令
    "preandroid:dev": "cross-env APP_ENV=dev npm run gen:env",

    // 运行项目，读取dev环境变量文件
    "android:dev": "cross-env APP_ENV=dev npx expo run:android --device",

    "preandroid:prod": "cross-env APP_ENV=prod npm run gen:env",

    // 运行项目，读取prod环境变量文件
    "android:prod": "cross-env APP_ENV=prod npx expo run:android --device",

    // 打包项目为apk文件，读取prod环境变量文件
    "android:build:apk": "cross-env APP_ENV=prod npm run gen:env && npm run android:gradle:apk",

    // 打包项目为aab文件，读取prod环境变量文件
    "android:build:aab": "cross-env APP_ENV=prod npm run gen:env && npm run android:gradle:aab",

    // 使用gradle进行打包
    "android:gradle:apk": "cd android && cross-env APP_ENV=prod ./gradlew assembleRelease",

    "android:gradle:aab": "cd android && cross-env APP_ENV=prod ./gradlew bundleRelease"
  },
}
```
